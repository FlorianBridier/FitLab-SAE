...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _loadDailyActivity() async {
    ...
    try {
      final response = await supabase
          ...
          .select('calories, workouts, steps')
          .eq('user_id', userId)
          .eq('date', today)
          .maybeSingle();
      ...
      if (response != null && mounted) {
        ...
        await _updateGoalsProgress(_steps);
      }
    } catch (e) {
  ...
  Future<void> _initPedometer() async {
    if (kIsWeb) {
      debugPrint('Pedomètre désactivé sur Web');
      return;
    }
    if (await Permission.activityRecognition.request().isGranted) {
      try {
        ...
        if (mounted) {
          ...
          await _updateGoalsProgress(savedSteps);
        }
        ...
        _stepSubscription = Pedometer.stepCountStream.listen((event) async {
          ...
          if (mounted) {
             ...
             await _updateGoalsProgress(dailySteps);
             await _saveWalkingData(dailySteps);
          }
        }, onError: (error) {
        ...
        child: Stack(
          children: [
            ...
            Column(
              children: [
                // --- CORRECTION ICI ---
                // On met le contenu du haut dans un Container avec une hauteur minimum de 340px.
                // Cela force le bloc suivant ("Accès Rapide") à commencer APRES le fond bleu.
                Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              Row(children: [Image.asset('/images/logo.png', height: 32, errorBuilder: (_,__,___) => const Icon(Icons.fitness_center, color: Colors.white)), const SizedBox(width: 10), const Text('FitLab', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 22))]),
                              Row(children: [
                ...
                // Le reste du contenu commence maintenant proprement après la courbe
                Padding(
